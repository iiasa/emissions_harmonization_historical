...
for stype, model_scen_match in scenario_model_match.items():
    ...
    if stype == "MOS":
        ...
        print(scenarios_global_mos.index.names)
        ...
        print(scenarios_global_mos.head())
        print(scenarios_regional_mos.loc[pix.ismatch(model=f"{model}", scenario=f"{scenario}")].shape)
...
for name, df_afolu in afolu_dfs.items():
    ...
    for s, meta in scenario_model_match.items():
        print(s)
        print(meta)
        print(scenarios_complete_global.columns)
        ...
        print(df_afolu)
        print(df_afolu.columns)
        print(df_afolu.head())
        print(name)
        co2_afolu = df_afolu.loc[(df_afolu["model"] == meta[1]) & (df_afolu["scenario"] == meta[0])]

        print(co2_afolu.shape)
        print(scenario_model_match[s])
